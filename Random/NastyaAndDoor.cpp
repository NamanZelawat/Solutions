...
//Mixing water
//See the macros..
//Just try to understand, you can get the code on github 
...
int main()
{
	ll t;
	cin>>t;
	while(t--)//Considering all the test cases
	{
		dd h,c,temp;
		cin>>h>>c>>temp; //Input as given in question
		//Observe that it is making a series. the given terms.
		...
		//For an even number term it's always (h+c)/2 which is 
		//the smallest term of the series.(from above)
		
		//For odd number of terms it will be ((n+1)*h+n*c)/(2*n+1) ---(1)  here total number 
		//of cups poured is (2*n+1) --- (2).
		//See that total number of terms is always odd.
		
		//Now calculating n from the above formula and using temp 
		//as we have to minimize the difference with temp.
		...
		//1 ----->   if(temp <=  (h+c)/2)(Smallest term as proved above) then 
		//ans will be an even number
		//and as we have to print minimum so ans will be 2
		//Explaination for first if statement
		...
		//And we have to check if the next odd term after 2*n + 1 
		//which is 2*n + 3 has lesser differnce
		//than the current odd term(If the temp lies between any 
		//two terms the formula will give the lower term)
		//So we have to check if the upper term is more closer or lower.
		
		//So one more formula  
		//if (  abs(((n+1)*h + n*c)/2*n+1 - t)  <   abs(((n+2)*h + (n+1)*c)/2*n+3 - t)     )
		//You can pause and see the formula
		//Slightly solving this equation gives us
		//(   abs(    ((n+1)*h + n*c)    -    (2*n+1)*t    )/2*n+1       <    
		// abs(    ((n+2)*h + (n + 1)*c)    -    (2*n+3)*t    )/2*n+3  )
		//
		//Cross multiply positive values gives
  		//(   abs(    ((n+1)*h + n*c)    -    (2*n+1)*t    )*(2*n+3)       <    
  		// abs(    ((n+2)*h + (n + 1)*c)    -    (2*n+3)*t    )*(2*n+1)  )
  		//then ans will be 2*n+1
  		//else ans will be 2*n+3
  		
  		//Code goes here
  		if( (h + c) - 2*temp >=0)//See the above explanation
  		{
  			cout<<2<<endl;
  		}
  		else
